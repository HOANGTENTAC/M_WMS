...
namespace M_WMS.ViewModel
{
    public partial class ReadFileExcelViewModel : ObservableObject
    {
        ...
        string inoutNo = "", InstrNoOld = "", StockKuOld = "";
        ...
        private async Task PickAndReadExcelAsync()
        {
            try
            {
                if (IsInputSelect)
                {
                    await ExecuteInput(_popupService, _apiService);
                ...
                else
                {
                    ...
                    //await res.Execute(_popupService, _apiService);
                    StatusText = $"Đã đọc thành công {StockInoutTenplate.Count} dòng từ Sheet '{CurrentErpUtil.LoadSheetOutput(LoginInfo.KyotenCd)}'!";
                }
            }
            catch (Exception ex)
            {
                await _popupService.ShowPopupAsync(AppResources.ErrorAlert, ex.Message, PopupType.Warning, "OK");
            }
        }
        ...
        public ReadFileExcelViewModel(ApiService apiService, IPopupDialogService popupService)
        {
            ...
            _isEnTable = CurrentErpUtil.IsEnTable(LoginInfo.KyotenCd);
        }

        public async Task<bool> ExecuteInput(IPopupDialogService _popupService, ApiService _apiService)
        {
            try
            {
                ...
                var dtzaiko_ku = await _apiService.SelectDbDictionaryCmms(sqlzaiko_ku);
                ...
                await Task.Run(async () =>
                {
                    ...
                    foreach (var row in worksheet.RowsUsed())
                    {
                        ...
                        string Qty = row.Cell("H").GetValue<string>();
                        InstrNo = CurrentErpUtil.ReplaceSpace(row.Cell("K").GetValue<string>());
                        string zaikoKu = string.IsNullOrEmpty(CurrentErpUtil.ReplaceSpace(StockType)) ? "" : StockType.Substring(0, 2);
                        string Hinmei = "";
                        string Unit = "";
                        string Note = row.Cell("J").GetValue<string>();
                        //string LotNo = Range("K" + j);
                        string ExpirationDate = row.Cell("L").GetValue<string>() != "" ? DateTime.Parse(row.Cell("L").GetValue<string>()).ToString("yyyy/MM/dd") : "";
                        ...
                        var currentMessage = new StockInoutTenplate
                        {
                            Message = "Đang đọc Row số " + j.ToString(),
                            StockType = StockType,
                            GoosdCd = ShoCd,
                            Qty = int.Parse(Qty),
                            InstrNo = InstrNo,
                            // Gán các thuộc tính khác của dữ liệu Excel vào đây...
                        };

                        MainThread.BeginInvokeOnMainThread(() =>
                        {
                            StockInoutTenplate.Add(currentMessage);
                            StatusText = $"Đã tải: {StockInoutTenplate.Count} dòng...";
                        });
                        ...
                        FROM mst_cr_shos MCS
                        JOIN mst_cr_sho_kyotens MCSK ON MCSK.mst_cr_shos_id = MCS.id AND MCSK.kyoten_cd = '{LoginInfo.KyotenCd}' AND MCSK.void_flg = 0
                        LEFT JOIN mst_cr_sho_stock MCSST ON MCSST.sho_cd = MCS.sho_cd AND MCSST.stock_ku = '{(StockType == "" ? "" : StockType.Substring(0, 2))}' AND MCSST.void_flg = 0 and MCSST.status = 1 and MCSST.kyoten_cd = '{LoginInfo.KyotenCd}' and MCSST.stock_location = '{LoginInfo.StockLocation}'
                        WHERE MCS.void_flg = 0 AND MCS.sho_cd = '{ShoCd}'";
                        }
                        var checkShoCdData = await _apiService.SelectDbDictionaryCmms(checkShoCd);
                        ...
                        if (InstrNo != InstrNoOld || zaikoKu != StockKuOld)
                        {
                            ...
                            if (dtzaiko_ku.Where(x => x["zaiko_ku_cd"].ToString() == zaikoKu).Count() == 0 || StockType == "")
                            {
                                res = false;
                                await _popupService.ShowPopupAsync(AppResources.ErrorAlert, string.Format(AppResources.WMS9999CW0074, ShoCd) + "\r\n" + "Line : " + j, PopupType.Warning, "OK");
                                return res;
                            }

                            var stocktypeOld = await CurrentErpUtil.LoadStorageType(_apiService, zaikoKu, culture: CurrentErpUtil.CultureKyoten(LoginInfo.KyotenCd), onlyname: true);
                            StockTypeName = stocktypeOld[0]["name"].ToString();

                            if (checkShoCdData.Count == 0)
                            {
                                res = false;
                                await _popupService.ShowPopupAsync(AppResources.ErrorAlert, string.Format(AppResources.WMS9999CW0056, ShoCd) + " - " +
                            ...
                            var dataTable = await SqlServerService.GetDataFromRegionAsync(getposql, LoginInfo.KyotenCd);
                            ...
                            //inoutNo = (await _apiService.GetArrivalStockiOrderNumber(LoginInfo.KyotenCd)).PadLeft(8, '0');
                            ...
                            regexItem.IsMatch(CurrentErpUtil.ReplaceSpace(InstrNo).Substring(0, 1)))
                        {
                            ...
                            {
                                res = false;
                                await _popupService.ShowPopupAsync(AppResources.ErrorAlert, string.Format(AppResources.WMS9999CW0044, ShoCd) + "\r\n" + "Line : " + j);
                                return res;
                            }
                            // check line no
                            if (string.IsNullOrEmpty(LineNo))
                            {
                                ...
                                DataTable dtCheckLineNo = await SqlServerService.GetDataFromRegionAsync(sqlCheckLineNo, LoginInfo.KyotenCd);
                                if (int.Parse(dtCheckLineNo.Rows[0][0].ToString()) > 1)
                                {
                                    res = false;
                                    await _popupService.ShowPopupAsync(AppResources.ErrorAlert, string.Format(AppResources.WMS9999CW0078, ShoCd) + "\r\n" + "Line : " + j);
                                    return res;
                                }
                                else
                                {
                                    DataTable dtGetLineNo = await SqlServerService.GetDataFromRegionAsync(sqlGetLineNo, LoginInfo.KyotenCd);
                            ...
                            Hinmei = checkShoCdData[0]["sho_name"].ToString();
                            Unit = checkShoCdData[0]["tani"].ToString();
                            int qtyPlan = 0;
                            int qtyHistory = 0;
                            ...
                            string sqlHistory = $@"select sum(d.suu) as suu
                                                ...
                                                {(LoginInfo.KyotenCd == "KZ" ? $"and d.stock_location = '{LoginInfo.StockLocation}'" : "")}";

                            var dthistory = await _apiService.SelectDbDictionaryErp(sqlHistory);

                            if (dthistory.Count > 0)
                            {
                                qtyHistory = int.Parse(string.IsNullOrEmpty(dthistory[0]["suu"].ToString()) ? "0" : dthistory[0]["suu"].ToString());
                            }
                            ...
                            dtPlan = await SqlServerService.GetDataFromRegionAsync(sqlPlan, LoginInfo.KyotenCd);
                            ...
                            {
                                var conf = await _popupService.ShowPopupAsync(AppResources.ErrorAlert, AppResources.WMS9999CW0084 + "\r\n" + "Line : " + j, PopupType.Question);
                                if (conf == false)
                                {
                                    res = false;
                                    return res;
                            ...
                            var tblInoutHistoryD = new TblInoutHistoryD
                            {
                                ...
                                Unit = Unit,
                                Suu = int.Parse(Qty),
                                GrpCd = checkShoCdData[0]["grp_cd"].ToString(),
                                ZaikoKu = zaikoKu,
                                InoutReason = Reason,
                                UpdatedAt = DateTimeOffset.Now,
                                UpdatedUserCd = LoginInfo.UserCd,
                                ...
                                InoutNo = inoutNo,
                                MstCrShoKyotensId = int.Parse(checkShoCdData[0]["kyotenId"].ToString()),
                                StockBasho = checkShoCdData[0]["stock_basho"].ToString(),
                                ...
                                StockQty = int.Parse(checkShoCdData[0]["stock_suu"].ToString()),
                            };
